...
class GraphButtonView(View):
    def __init__(self):
        ...
        self.add_item(Button(label="起床グラフを生成", style=discord.ButtonStyle.success, custom_id="graph2_button"))
...
async def on_ready():
    ...
    for guild in bot.guilds:
        ...
        if graph_channel:
            embed = discord.Embed(title="グラフ生成", description="ボタンを押して自分の睡眠グラフや起床グラフを生成してください。", color=0x0000ff)
...
async def on_interaction(interaction: discord.Interaction):
    ...
    elif interaction.data["custom_id"] == "graph2_button":
        # 起床グラフを生成
        author = interaction.user.name
        graph_path = await generate_graph2(author)
...
async def generate_graph(user_name):
    if not os.path.exists(SLEEP_CSV_FILE):
        return None

    dates, sleep_times = [], []
    hour_distribution = [0] * 24  # 時刻ごとの投稿数を格納するリスト
    now = datetime.now(JST)
    ...
    with open(SLEEP_CSV_FILE, 'r', encoding='utf-8') as csvfile:
        ...
        for row in csv_reader:
            ...
            if four_months_ago <= date <= now and author == user_name:
                dates.append(date)
                sleep_hour = date.hour  # 睡眠時間を取得
                sleep_times.append(sleep_hour)
                hour_distribution[sleep_hour] += 1  # ユーザーごとの投稿数を集計

    if not dates:
        return None

    # グラフの設定と描画
    plt.figure(figsize=(20, 14))  # サイズ調整
    ...
    for i in range(1, len(dates)): 
        ...
        if (dates[i] - dates[i - 1]).days > 1:  
            plt.plot(dates[i-1:i+1], sleep_times[i-1:i+1], marker='o', markersize=2, linestyle='None', color='b')  
        else: 
            plt.plot(dates[i-1:i+1], sleep_times[i-1:i+1], marker='o', markersize=2, linestyle='-', color=line_color, alpha=0.7) 
    ...
    plt.title(f'{user_name}さんの睡眠周期記録グラフ')  
    plt.xlabel('')  
    plt.ylabel('睡眠時刻（時）')  
    ...
    plt.subplot(2, 1, 2)
    hours = range(24)
    plt.bar(hours, hour_distribution, color='skyblue', alpha=0.7)
    plt.xticks(hours)
    plt.xlabel('睡眠時刻（時）')
    plt.ylabel('頻度')
    plt.title(f'{user_name}さんの睡眠時刻の分布')
    
    # グラフ間の余白を設定
    plt.subplots_adjust(hspace=0.4, top=0.95, bottom=0.05, left=0.05, right=0.95)

    # グラフ全体を保存
    graph_path = f"{user_name}_sleep_graph_with_distribution.png"  
    plt.savefig(graph_path)  
    plt.close()  
    return graph_path

async def generate_graph2(user_name):
    if not os.path.exists(WAKE_CSV_FILE):
        return None
    ...
    with open(WAKE_CSV_FILE, 'r', encoding='utf-8') as csvfile:
        ...
        for row in csv_reader:
            ...
            if four_months_ago <= date <= now and author == user_name:
                dates.append(date)
                wake_hour = date.hour  # 起床時間を取得
                wake_times.append(wake_hour)
                hour_distribution[wake_hour] += 1  # ユーザーごとの投稿数を集計
    ...
    for i in range(1, len(dates)): 
        ...
        if (dates[i] - dates[i - 1]).days > 1:  
            plt.plot(dates[i-1:i+1], wake_times[i-1:i+1], marker='o', markersize=2, linestyle='None', color='b')  
        else: 
            plt.plot(dates[i-1:i+1], wake_times[i-1:i+1], marker='o', markersize=2, linestyle='-', color=line_color, alpha=0.7) 
    ...
    plt.title(f'{user_name}さんの起床周期記録グラフ')  
    plt.xlabel('')  
    plt.ylabel('起床時刻（時）')  
    plt.xticks(rotation=45)  
    plt.xlim([four_months_ago, now])  
    ...
    plt.xlabel('起床時刻（時）')
    plt.ylabel('頻度')
    plt.title(f'{user_name}さんの起床時刻の分布')
    
    # グラフ間の余白を設定
    plt.subplots_adjust(hspace=0.4, top=0.95, bottom=0.05, left=0.05, right=0.95)

    # グラフ全体を保存
    graph_path = f"{user_name}_wake_graph_with_distribution.png"  
    plt.savefig(graph_path)  
    plt.close()  
    return graph_path
